...
CREATE TABLE IF NOT EXISTS periods (period_id INTEGER PRIMARY KEY, period_date TEXT, display_label TEXT NOT NULL );
...
CREATE TABLE IF NOT EXISTS auctions (auction_id TEXT PRIMARY KEY, label TEXT NOT NULL, bid_close_label TEXT NOT NULL, status TEXT NOT NULL, created_at TEXT NOT NULL, ran_at TEXT, auction_date TEXT, daysInPeriod INTEGER, numberOfPeriods INTEGER, auction_type TEXT );
CREATE TABLE IF NOT EXISTS auction_periods (auction_id TEXT NOT NULL, period_id INTEGER NOT NULL, period_order INTEGER NOT NULL, PRIMARY KEY (auction_id, period_id), FOREIGN KEY (period_id) REFERENCES periods(period_id) );
...
CREATE TABLE IF NOT EXISTS default_control_point_bounds (control_point_id TEXT NOT NULL, period_id INTEGER NOT NULL, bound REAL NOT NULL, PRIMARY KEY (control_point_id, period_id), FOREIGN KEY (control_point_id) REFERENCES control_points(control_point_id) );
CREATE TABLE IF NOT EXISTS control_point_bounds (auction_id TEXT NOT NULL, control_point_id TEXT NOT NULL, period_id INTEGER NOT NULL, bound REAL NOT NULL, PRIMARY KEY (auction_id, control_point_id, period_id), FOREIGN KEY (control_point_id) REFERENCES control_points(control_point_id), FOREIGN KEY (period_id) REFERENCES periods(period_id) );
...
CREATE TABLE IF NOT EXISTS auction_runs (run_id INTEGER PRIMARY KEY AUTOINCREMENT, auction_id TEXT NOT NULL, clearing_start_time TEXT, run_at TEXT NOT NULL, solve_status TEXT NOT NULL, objective_value REAL NOT NULL );
CREATE TABLE IF NOT EXISTS accepted_bids (run_id INTEGER NOT NULL, bid_id INTEGER NOT NULL, accepted_quantity REAL NOT NULL, PRIMARY KEY (run_id, bid_id), FOREIGN KEY (run_id) REFERENCES auction_runs(run_id), FOREIGN KEY (bid_id) REFERENCES bids(bid_id) );
CREATE TABLE IF NOT EXISTS period_prices (run_id INTEGER NOT NULL, period_id INTEGER NOT NULL, price REAL NOT NULL, PRIMARY KEY (run_id, period_id), FOREIGN KEY (run_id) REFERENCES auction_runs(run_id), FOREIGN KEY (period_id) REFERENCES periods(period_id) );
...
CREATE TABLE IF NOT EXISTS response_matrix (coefficient_id INTEGER PRIMARY KEY AUTOINCREMENT, RMI_id INTEGER, wellid TEXT, control_point_id TEXT, RM_take_of_water_date TEXT, RM_effect_on_control_point_date TEXT, RM_impact_coefficient REAL, FOREIGN KEY (RMI_id) REFERENCES response_matrix_info(RMI_id) );
...
def db_status(db_path: Path) -> dict:
	...
	try:
		...
		if "control_points" in tables:
			counts["cps_with_lat"] = conn.execute("SELECT COUNT(*) FROM control_points WHERE latitude IS NOT NULL").fetchone()[0]
		conn.close()
...
def _decode_cp(token: str, num_control_periods: int) -> tuple[int, int]:
	...
	if m_num: idx = int(m_num.group(1))
		...
		idx = int(m_bon.group(1))
	else:
		raise ValueError(f"Cannot decode CP token: {token!r}")
...
def import_hedcon(db_path: Path, text: str, num_control_points: int | None = None, num_control_periods: int | None = None) -> dict:
	...
	RHS_head values are stored in default_control_point_bounds (no auction_id).
	Auction-specific bounds in control_point_bounds are populated from the
	Hydrologist maintenance page for a specific open auction.
	...
	for line in text.splitlines():
		...
		try:
			...
			if rhs_head is not None:
				conn.execute("INSERT OR REPLACE INTO default_control_point_bounds" "(control_point_id, period_id, bound) VALUES (?,?,?)", (cp_id, effect_period, rhs_head))
				bounds_inserted += 1
		except Exception as exc:
			errors.append(str(exc))
	conn.commit()
...
def import_mps(db_path: Path, text: str) -> dict:
	"""Parse a GWM2K .mps file; insert response_factors, control_point_bounds, and wells.
	...
	for raw_line in text.splitlines():
		...
		if section == "COLUMNS":
			...
			try:
				...
				well_id = f"gwm-well-{well_num}"
				cp_id   = f"gwm-cp-{cp_num}"
				conn.execute("INSERT OR REPLACE INTO response_factors" "(well_id, control_point_id," " pumping_period, effect_period, factor_value)" " VALUES (?,?,?,?,?)", (well_id, cp_id, pump_period, effect_period, coef))
				rf_count += 1
			except Exception as exc:
			...
			try:
				cp_num, effect_period = _decode_cp(row_token, num_control_periods)
				cp_id = f"gwm-cp-{cp_num}"
				conn.execute("INSERT OR REPLACE INTO default_control_point_bounds" "(control_point_id, period_id, bound) VALUES (?,?,?)", (cp_id, effect_period, val))
				bound_count += 1
			except Exception as exc:
	...
	conn.commit()
	conn.close()
	return {"response_factors_inserted": rf_count, "control_point_bounds_inserted": bound_count, "well_rights_inserted": right_count, "num_wells": num_wells, "num_pump_periods": num_pump_periods, "num_control_points": num_control_points, "num_control_periods": num_control_periods, "errors": errors[:20],}
...
def apply_default_bounds(db_path: Path, auction_id: str) -> dict:
	...
	for cp_id, period_id, bound in rows:
		conn.execute("INSERT OR REPLACE INTO control_point_bounds" "(auction_id, control_point_id, period_id, bound) VALUES (?,?,?,?)", (auction_id, cp_id, period_id, bound),)
		count += 1
	conn.commit()
	conn.close()
	return {"bounds_applied": count, "auction_id": auction_id}
