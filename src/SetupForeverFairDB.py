...
CREATE TABLE IF NOT EXISTS control_point_events (cpe_id INTEGER PRIMARY KEY AUTOINCREMENT, control_point_id INTEGER, auction_id INTEGER, effect_date TEXT, committed_head_auction_start REAL, committed_head_auction_end REAL, committed_allowable_head_change REAL, planned_head_auction_start REAL, planned_head_auction_end REAL, slack REAL, dual_price REAL, alpha REAL, FOREIGN KEY (auction_id) REFERENCES auctions(auction_id), FOREIGN KEY (control_point_id) REFERENCES control_points(control_point_id));
...
# A responsible hydrologist should censor the bad data. The Tianqiao data has dodgy response matrix values for lags over 22 periods.
# Those values are relatively big and positive. In fact, the largest response matrix value is 9.533e-0.7 with lag 50 periods, absurd. 
# 	select (effect_period - pumping_period) as lag, max(response) from response_matrix
# Here, I clip response values for lags of 23+ weeks to the largest response over 22 weeks. Now the largest lag is 4.377e-0.7 with lag 1. Still weird.
# Anyways, this code lets you censor the response matrix. You should really fix your MODFLOW model, though.
RESPONSE_STATS: dict[str, float] = {"max": 0.0}
def reset_response_stats() -> None: RESPONSE_STATS["max"] = 0.0
def censor_the_response_matrix(well_id: int, control_point_id: int, pumping_period: int, effect_period: int, response: float) -> float:
	if effect_period - pumping_period <= 22: 
		RESPONSE_STATS["max"] = max (RESPONSE_STATS["max"], response)
	else: response = min (response, RESPONSE_STATS["max"])
	return response
	...
	for raw_line in text.splitlines():
		...
		if section == "COLUMNS":
			...
			try:
				...
				response = censor_the_response_matrix(well_id, cp_id, pump_period, effect_period, coef)
				conn.execute("INSERT OR REPLACE INTO response_matrix (well_id, control_point_id, pumping_period, effect_period, response) VALUES (?,?,?,?,?)", (well_id, cp_id, pump_period, effect_period, response))
				rf_count += 1
			except Exception as exc:
...
def create_tiny_demonstration (db_path: Path, lp_path: Path | None = None, period_length_hours: int = 168) -> dict[str, Any]:
	...
	for pumping_period in range(1, num_pumping_periods + 1):
		...
		for (well_id, control_point_id), vector in base_response_vectors.items():
			for effect_period in range(1, num_effect_periods + 1):
				...
				response = vector[idx]
				if 0.0 == response: continue
				response_rows.append((well_id, control_point_id, pumping_period, effect_period, response))
	conn.executemany("INSERT INTO response_matrix(well_id, control_point_id, pumping_period, effect_period, response) VALUES (?,?,?,?,?)", response_rows)
	...
	# Add head limits.
	conn.execute("DELETE FROM aquifer_head_limits")
	aquifer_rows: list[tuple[int, str, str, float, float, float]] = []
	for effect_date in pumping_date:
		aquifer_rows.append((1, effect_date, upload_date, 100.0, 80.0, 80.0 - 100.0))
		aquifer_rows.append((2, effect_date, upload_date, 120.0, 108.0, 108.0 - 120.0))
	conn.executemany ("INSERT INTO aquifer_head_limits(control_point_id, effect_date, upload_date, actual_start_head, minimum_head, allowable_head_change) VALUES (?,?,?,?,?,?)", aquifer_rows,)

	# Assert consistency for demonstration data: stored allowable_head_change must match minimum_head - actual_start_head.
	row = conn.execute("""SELECT COUNT(*) FROM aquifer_head_limits
		WHERE ABS(COALESCE(allowable_head_change, 0.0) - (minimum_head - actual_start_head)) > 1.0e-9""").fetchone()
	assert row is not None and int(row[0]) == 0, "Tiny demonstration aquifer_head_limits has inconsistent allowable_head_change values"
...
def setup_tiny_demonstration() -> Path:
	...
	summary = create_tiny_demonstration(db_path, period_length_hours=168)
