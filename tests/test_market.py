...
def _seed_auction_id(foreverFairData_instance: ForeverFairData) -> str: return foreverFairData_instance.load().auction.id

def test_market_clears_seed_case(tmp_path):
	foreverFairData_instance = _make_repo(tmp_path)
	auction_case = foreverFairData_instance.load()
...
def test_setup_and_reset_auction_data(tmp_path):
	...
	updated_auction = SetUpAuction(foreverFairData_instance, auction_id=None, closed_date=close_dt, first_water_take_date=first_take, last_water_take_date=last_take, period_length_hours=168,)
	assert updated_auction.id is not None
	assert len(foreverFairData_instance.list_auctions()) == 2

	reset_case = ResetAuctionData(foreverFairData_instance)
...
def test_multistep_bid_creates_multiple_lp_variables(tmp_path):
	...
	first_step = foreverFairData_instance.add_bid(auction_id=auction_id, trader_id=participant_id, well_id=well_id, period_id=period_key, quantity=6.0, price=20.0, bid_steps=[(6.0, 20.0), (4.0, 14.0), (2.0, 9.0)],)
...
def test_get_quota_uses_licenses_and_final_quota(tmp_path):
	...
	quota = foreverFairData_instance.get_quota(trader_id=participant_id, period_start=period_key, auction_id=auction_id)
